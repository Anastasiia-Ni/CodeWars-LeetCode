...
but 101-107is not a 6-gap because there is 103in between and 103-109
...
int check_gap(long long i, int g)
{
...
long long* gap(int g, long long m, long long n) {
  long long*arr = calloc(2, sizeof(long long));
  for (long long i = m; i <= (n - g); i++)
  {
    if (is_prime(i) && is_prime(i + g) && check_gap(i, g))
    {
        arr[0] = i;
        arr[1] = i + g;
        break;
