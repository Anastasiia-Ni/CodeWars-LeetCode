...
# have the same length sorted in ascending lexicographic order (letters and digits - more 
# precisely sorted by codepoint); the different groups will be separated by '/'. 
    ...
    for c in "abcdefghijklmnopqrstuvwxyz":
        ...
        if max(i1, i2) > 1:
            if i1 > i2:
                ...
                lit = "="
            tmp[c] = (-1 * max(i1, i2), lit + ":" + c * max(i1, i2))
    return "/".join(tmp[c][1] for c in sorted(tmp, key=lambda x: tmp[x]))