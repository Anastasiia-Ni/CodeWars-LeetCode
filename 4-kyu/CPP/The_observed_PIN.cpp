...
std::unordered_map<char, std::vector<char>> buttons = {
  {'0', {'0', '8'}},
  {'1', {'1', '2', '4'}},
  {'2', {'1', '2', '3', '5'}},
  {'3', {'2', '3', '6'}},
...
void generateCombinations(std::vector<std::vector<char>> &sets, std::vector<std::string> &result, std::string& combination, size_t index) {
    if (index == sets.size()) {
        result.push_back(combination);
        return;
    }
    for (char c : sets[index]) {
        combination.push_back(c);
        generateCombinations(sets, result, combination, index + 1);
...
std::vector<std::string> get_pins(std::string observed) {
  ...
  for (char c : observed) {
    sets.push_back(buttons[c]);
  }
  generateCombinations(sets, result, combination, 0);
