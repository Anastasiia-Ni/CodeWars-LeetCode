...
If there are multiple such values, it returns the value associated with t
he largest timestamp_prev. If there are no values, it returns "".
...
public:
    ...
    void set(string key, string value, int timestamp) {
        auto p1 = make_pair(timestamp, value);
        um[key].push_back(p1);
    }
    ...
    string get(string key, int timestamp) {
        string ret = "";
        int start = 0;
        vector <pair <int, string> >& tmp = um[key];
        int end = tmp.size() - 1;
        while (start <= end) {
